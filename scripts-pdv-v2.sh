...
# ==========================================
# SUBMENU 5 - AGENDAMENTO (POWER)
# ==========================================
menu_power() {
    while true; do
        echo -e "\n${BLUE}--- SUBMENU ENERGIA/DESLIGAMENTO ---${NC}"
        echo "1 - Agendar Desligamento Diário (00:30)"
        echo "2 - Remover Agendamento Diário"
        echo "3 - Ver Agendamentos Ativos (Crontab)"
        echo "0 - Voltar"
        read -p "Opção: " o_pow
        case $o_pow in
            1)
                # Adiciona ao crontab do root se não existir
                (sudo crontab -l 2>/dev/null | grep -v "shutdown -h now" ; echo "30 00 * * * /sbin/shutdown -h now") | sudo crontab -
                echo -e "${GREEN}Desligamento agendado para as 00:30 diariamente.${NC}" ;;
            2)
                # Remove a linha do crontab do root
                sudo crontab -l 2>/dev/null | grep -v "shutdown -h now" | sudo crontab -
                echo -e "${RED}Agendamento de desligamento removido.${NC}" ;;
            3)
                echo -e "\n${BLUE}Tarefas agendadas no Root:${NC}"
                sudo crontab -l 2>/dev/null || echo "Nenhuma tarefa encontrada." ;;
            0) break ;;
        esac
    done
}
...
while true; do
    ...
    echo "5) ENERGIA (Desligamento Automático)"
    echo "0) Sair"
    echo -e "${BLUE}==========================================${NC}"
